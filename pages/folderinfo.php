...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'delete_document') {
  $doc_id = intval($_POST['document_id']);
  $stmt = $conn->prepare("UPDATE documents SET deleted_at = NOW() WHERE id = ?");
  $stmt->bind_param("i", $doc_id);
  $stmt->execute();
  echo json_encode(['status' => 'success']);
  exit();
...
<body class="g-sidenav-show  bg-gray-100">
    ...
    <div class="container-fluid py-2">
      ...
      <div class="row mb-4">
        ...
        <div class="col-md-6 d-flex justify-content-md-end align-items-center mt-3 mt-md-0">
          ...
          <?php } ?>  
           <?php 
                if (Utils::canUploadDocuments($conn, $user_id) || Utils::isSuperadmin($conn, $user_id)) {
          ?>
  ...
  Swal.fire({
    ...
    text: `Are you sure you want to delete "${docTitle}"? This cannot be undone.`,
    icon: 'warning',
    showCancelButton: true,
    confirmButtonText: 'Delete',
    cancelButtonText: 'Cancel'
